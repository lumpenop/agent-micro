...
- 로컬 Git의 변경 파일·스테이징·커밋을 처리하고, 선택적으로 GitHub를 연결해 Pull·Push를 사용합니다.
...
2. **GitHub 연결(선택)**: 기존 GitHub 로그인을 확인하고, 필요하면 브라우저에서 연결합니다. GitHub 기능이 필요하지 않으면 `건너뛰기`를 선택해도 됩니다.
3. **프로젝트 선택**: Codex가 작업할 폴더를 선택합니다.
4. **Agent 시작**: `시작하기`를 눌러 첫 번째 Codex 세션을 엽니다.

같은 Mac에서 Codex CLI나 GitHub CLI에 이미 로그인했다면 해당 단계가 자동으로 완료될 수 있습니다. GitHub 연결을 건너뛰어도 로컬 Git과 기본 Codex 기능은 사용할 수 있으며, 나중에 **Settings → Account → GitHub**에서 연결할 수 있습니다. 작업 폴더와 고급 기능도 Settings에서 바꿀 수 있습니다.
...
- Codex 로그인 및 연결 상태
- 선택형 GitHub 로그인 및 연결 상태
- Provider와 API 설정
- 모델과 추론 강도
- 새 Agent를 답변·분석 전용 읽기 모드로 여는 Ask 모드
- 플랜 사용 퍼센트 기준 일일 목표·차단선과 리셋까지의 권장 사용량
- 질문 복잡도와 최근 실제 토큰 사용 기록에 따른 자동 모델·추론 강도 선택
- 다음 요청의 예상 토큰 범위와 일일/플랜 목표 초과 사전 경고
- 작업 폴더
